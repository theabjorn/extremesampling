...
```{r}
    ...
    epsCC.test(y~xe1+xe2, SNP=cbind(xg1,xg2), cutoffs=c(l,u))$p.value
    # or using matrix form
    # epsCC.test(y~xe, SNP=xg, cutoffs=c(l,u))$p.value
```
...
```{r}
    ...
    epsCC.test(y~xe1+xe2+xg1+xg2, SNP = xe1*xg1, cutoffs=c(l,u))$p.value
```
...
```{r}
    ...
    epsAC.test(y~xe1+xe2, SNP=cbind(xg1,xg2))$p.value
    # alternatively with matrix form
    epsAC.test(y~xe, SNP=xg)$p.value
```
...
```{r}
    ...
    epsAC.test(y~xe1+xe2, SNP=xg, confounder=TRUE, cx=c("xe2"))
    
    # Test (without confounding)
    epsAC.test(y~xe1+xe2, SNP=xg)
```
