...
(Only allowed when the number of SNPs is low)
...
For the **EPS-complete design**, the variables `y` and `xe` are observed for 
...
model fitting are based on a likelihood model for missing at random covariates
when the missing-mechanism is ignorable for likelihood inference. 

The following example will be used throughout for illustrating EPS-complete
functions. 
...
Below, we present a few key examples of testing for the EPS-complete design.
...
`y = a + be*xe + bg*xg + e` using data from the EPS-complete design.
...
#### EPS-complete and Hardy-Weinberg

The EPS-complete model is based on a likelihood model that incorporates the
distribution of the covariate with missing values. That means that the 
distribution of `xg` is assumed. In general, we assume that `xg` takes
values 0, 1 or 2 with (unknown) probabilities `p0`, `p1` and `p2`, where
`p2 = 1 - p0 - p1`. The user can however specify that Hardy-Weinberg 
equilibrium is assumed, in that case `xg` takes
values 0, 1 or 2 with (unknown) probabilities `(1-q)^2`, `2q(1-q)` and `q^2`
where `q` is the minor allele frequency. This frequency can be unknown or 
...
#### EPS-complete and confounding effects

If there are confounding effects in the data, then this must
be accounted for in the (unknown) distribution of `xg` in the EPS-complete 
...
confounders can only be discrete environmental covariates with few unique
levels.
